...
#Plots
  ...
  line_chart_server("vt", data_logbooks, colTarget="vesseltype",colValue="quantity", rank=FALSE)
  line_chart_server("sp", data_logbooks%>%
                        mutate(text=sprintf("%s-<em>%s</em>(<b>%s</b>)",species_desc,species_sci,species_asfis)), colTarget="species_desc",colValue="quantity",colText="text", rank=TRUE,nbToShow=5,rankLabel="Display x most catched species:")
  line_chart_server("fg", data_logbooks%>%left_join(fish_group), colTarget="ISSCAAP_Group_En",colValue="quantity", rank=FALSE)
