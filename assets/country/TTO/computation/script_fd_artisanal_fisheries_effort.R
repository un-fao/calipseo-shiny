...
compute_1st_raised_effort <- function(raw_data, raised_1){
  ...
  raised_1 <- as.data.frame(raised_1)
	...
	#unique trips
	raw_data_01_trips <- unique(raw_data_01[,c("landing_id","regnum","crew","bch_id","bch_name","f_mthd_id","f_mthd","year.x","month","raised_hours","raised_trips", "raised_mantrips")])
	
	#aggregate
	IGNORE_DIMENSION <- rep("all", nrow(raw_data_01_trips))
	
	#1st aggregation based on gear/month
	#---------------------------------------------------------------------------------------------------------------------------------
	...
	out_result_HOURS <- do.call(data.frame, aggregate(
		...
		by = list(
			...
			"gear_id" = raw_data_01_trips$"f_mthd_id",
			"gear_name" = raw_data_01_trips$"f_mthd",
	...
	#2nd aggregation based on gear (compute total yearly descriptors by gear)
	#---------------------------------------------------------------------------------------------------------------------------------
	...
	#3rd aggregation based on gear (compute total yearly descriptors by month)
	#---------------------------------------------------------------------------------------------------------------------------------
	#out aggregations
	out_result3_HOURS <- do.call(data.frame, aggregate(
		...
		by = list(
			"bch_id" = raw_data_01_trips$"bch_id",
			"bch_name" = raw_data_01_trips$"bch_name",
			"gear_id" = IGNORE_DIMENSION,
			"gear_name" = IGNORE_DIMENSION,
			"year" = raw_data_01_trips$"year.x",
			"month" = raw_data_01_trips$month
		),
		FUN = function(x){sum(x, na.rm = TRUE)}
	...
	out_result <- reshape(out_result, v.names = "var", idvar = c("bch_id", "bch_name", "gear_id", "gear_name", "year", "month"), timevar = "descriptor", times = sapply(names(out_result)[(ncol(out_result)-4):ncol(out_result)], function(x){unlist(strsplit(x,"var."))[2]}), varying = list(names(out_result)[(ncol(out_result)-4):ncol(out_result)]), direction = "long")
