...
artfish_report_server <- function(id, parent.session, pool, reloader){
 ...
 moduleServer(id, function(input, output, session){   
  ...
  ref_landing_sites<-NULL
  
  if(!is.null(minor_strata)){
    if(minor_strata=="landing_site"){
      ref_landing_sites <- accessLandingSites(pool)
      ref_landing_sites<-subset(ref_landing_sites, select=c(ID,NAME))
    }
  }
  ...
  estimate <- get_artfish_results_for_ui(files, ref_fishing_units, ref_species, ref_landing_sites, with_status=T)
