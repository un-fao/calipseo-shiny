...
s.NB_DAYS_SAMPLED as days_sampled, 
f.BG, 
f.NB_ACTIVE_DAYS 
FROM dt_effort_surveys as s 
LEFT JOIN cl_stat_effort_survey_types as st ON st.ID = s.CL_STAT_EFFORT_SURVEY_TYPE_ID 
LEFT JOIN(
SELECT 
YEAR, CL_APP_MONTH_ID, CL_FISH_FISHING_UNIT_ID, SUM(NB_FISHING_UNITS) as BG, MAX(NB_ACTIVE_DAYS) as NB_ACTIVE_DAYS FROM dt_frame_surveys 
GROUP BY YEAR, CL_APP_MONTH_ID, CL_FISH_FISHING_UNIT_ID 
) as f USING (CL_APP_MONTH_ID, YEAR,CL_FISH_FISHING_UNIT_ID) 
LEFT JOIN jt_fish_fishing_unit_segment_mapping as fufs ON fufs.CL_FISH_FISHING_UNIT_ID = f.CL_FISH_FISHING_UNIT_ID 
