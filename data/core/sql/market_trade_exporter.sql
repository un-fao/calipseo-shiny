SELECT
    ...
    cl_fish_markets.CODE AS market_code,
	  cl_fish_markets.I18N_DEFAULT AS market_name,
...
INNER JOIN cl_fish_markets 
    ON dt_market_visits.CL_REF_FISH_MARKET_ID = cl_fish_markets.ID 

INNER JOIN cl_ref_vendor_trade_types 
    ON dt_market_visit_details.CL_REF_VENDOR_TRADE_TYPE_ID = cl_ref_vendor_trade_types.ID 

INNER JOIN cl_fish_species s 
    ON dt_market_visit_details.CL_REF_SPECIES_ID = s.ID 

LEFT JOIN jt_fish_species_fish_species_group j 
    ON s.ID = j.CL_REF_SPECIES_ID 

LEFT JOIN cl_fish_species_groups sg 
    ON j.CL_REF_SPECIES_GROUP_ID = sg.ID 

LEFT JOIN cl_fish_species_group_types sgt 
